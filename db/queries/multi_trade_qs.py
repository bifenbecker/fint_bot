...
async def get_owner_selected_cards(ssn: AsyncSession, user_id, card_ids):
    cards_q = await ssn.execute(
        select(UserCard)
        .filter(UserCard.id.in_(card_ids))
        .filter(UserCard.user_id == user_id)
        .options(selectinload(UserCard.card))
    )
    cards = cards_q.scalars().all()
...
async def create_new_mtrade(
    ...
    u_trades_q = await ssn.execute(
        select(UserTrade).filter(UserTrade.user_x_user.in_(txts))
    )
    ...
    penalty_q = await ssn.execute(
        ...
        .filter(
            or_(
                Penalty.target.in_([user_id, target.id]),
                Penalty.owner.in_([user_id, target.id]),
            )
        )
...
async def get_user_rarity_cards_m_target(
    ssn: AsyncSession, user_id, rarity, sorting, trade_id
):
    ...
    owner_cards_q = await ssn.execute(
        select(TradeCard.card_id)
        .filter(TradeCard.trade_id == trade_id)
        .filter(TradeCard.kind == "owner")
    )
    ...
    if rarity == "all":
        if sorting == "up":
            cards_q = await ssn.execute(
                ...
                .filter(UserCard.user_id == user_id)
                .filter(UserCard.tradeble == "yes")
                .filter(UserCard.card_id.not_in(owner_cards))
                .order_by(UserCard.points)
                .options(selectinload(UserCard.card))
            )
        elif sorting == "down":
...
async def get_target_selected_cards(ssn: AsyncSession, trade_id, card_ids):
    ...
    owner_trade_cards_q = await ssn.execute(
        select(TradeCard.user_card_id)
        .filter(TradeCard.trade_id == trade_id)
        .filter(TradeCard.kind == "owner")
    )
    ...
    owner_cards_q = await ssn.execute(
        select(UserCard)
        .filter(UserCard.id.in_(owner_trade_cards))
        .filter(UserCard.user_id == trade.owner)
        .options(selectinload(UserCard.card))
    )
    owner_cards = owner_cards_q.scalars().all()
    ...
    target_cards_q = await ssn.execute(
        select(UserCard)
        .filter(UserCard.id.in_(card_ids))
        .filter(UserCard.user_id == trade.target)
        .options(selectinload(UserCard.card))
    )
...
async def change_mtrade_status(ssn: AsyncSession, trade_id, card_ids, user_id):
    ...
    if len(cards) < trade.quant:
        await ssn.execute(
            update(Trade).filter(Trade.id == trade_id).values(status="declined")
        )
        await ssn.commit()
        return "error"
    ...
    await ssn.execute(
        update(Trade).filter(Trade.id == trade_id).values(status="owner_wait")
    )
    await ssn.commit()
...
async def close_multi_trade(ssn: AsyncSession, trade_id):
    ...
    target_trade_cards_q = await ssn.execute(
        select(TradeCard.user_card_id)
        .filter(TradeCard.trade_id == trade_id)
        .filter(TradeCard.kind == "target")
    )
    target_trade_cards = target_trade_cards_q.scalars().all()

    target_cards_q = await ssn.execute(
        select(UserCard)
        .filter(UserCard.id.in_(target_trade_cards))
        .filter(UserCard.user_id == trade.target)
    )
    target_cards = target_cards_q.scalars().all()
    ...
    for t_o_card in target_cards:
        check_card_q = await ssn.execute(
            select(UserCard.id)
            .filter(UserCard.card_id == t_o_card.card_id)
            .filter(UserCard.user_id == trade.owner)
        )
        ...
        await ssn.execute(
            update(UserCard)
            .filter(UserCard.id == t_o_card.id)
            .values(user_id=trade.owner, duplicate=owner_duplicate)
        )
    ...
    for o_t_card in owner_cards:
        check_card_q = await ssn.execute(
            select(UserCard.id)
            .filter(UserCard.card_id == o_t_card.card_id)
            .filter(UserCard.user_id == trade.target)
        )
        check_card_res = check_card_q.scalars().all()
        if len(check_card_res) > 0:
        ...
        await ssn.execute(
            update(UserCard)
            .filter(UserCard.id == o_t_card.id)
            .values(user_id=trade.target, duplicate=target_duplicate)
        )
    ...
    owner_user_q = await ssn.execute(select(Player).filter(Player.id == trade.owner))
    ...
    target_user_q = await ssn.execute(select(Player).filter(Player.id == trade.target))
    ...
    await ssn.execute(
        update(Trade).filter(Trade.id == trade_id).values(status="finished")
    )
    await ssn.merge(UserTrade(user_x_user=f"{trade.owner}_{trade.target}"))
    await ssn.commit()
    logging.info(f"Traded {trade_id} | user1 {trade.owner} | user2 {trade.target}")
