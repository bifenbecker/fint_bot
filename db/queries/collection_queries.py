    ...
    if rarity == "all":
        if sorting == "up":
            cards_q = await ssn.execute(
                select(CardItem).order_by(CardItem.points))
        elif sorting == "down":
            cards_q = await ssn.execute(
...
async def re_count_duplicates(ssn: AsyncSession, user_id):
    await ssn.execute(update(UserCard).filter(
        UserCard.user_id == user_id).values(duplicate=0))
    await ssn.commit()

    cards_q = await ssn.execute(select(UserCard).filter(
        UserCard.user_id == user_id).order_by(UserCard.card_id))
    cards = cards_q.scalars().all()

    card: UserCard
    for card in cards:
        ucard_q = await ssn.execute(select(UserCard.id).filter(
            UserCard.card_id == card.card_id).filter(
                UserCard.user_id == user_id).filter(
                    UserCard.duplicate == 0).filter(
                        UserCard.id != card.id))
        ...
        if ucard_res is not None:
            if card.duplicate == 0:
                await ssn.execute(update(UserCard).filter(
                    UserCard.id == card.id).values(duplicate=1))
                await ssn.commit()
...
async def get_user_team_cards(ssn: AsyncSession, user_id, team, sorting):
    card_ids_q = await ssn.execute(select(CardItem.id).filter(
        CardItem.team.ilike(f"%{team}%")))
    card_ids = card_ids_q.scalars().all()

    if sorting == "up":
        cards_q = await ssn.execute(select(UserCard).filter(
            UserCard.user_id == user_id).filter(
                UserCard.card_id.in_(card_ids)).filter(
                    UserCard.duplicate == 0).order_by(
                        UserCard.points).options(
                            selectinload(UserCard.card)))
    elif sorting == "down":
