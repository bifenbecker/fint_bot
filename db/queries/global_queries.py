...
from db.models import CardItem, Games, Player, PlayerPick, UserCard, UserPacks
...
async def check_and_add_user(ssn: AsyncSession, user_id, username):
    ...
    if user_res is None:
        ...
        await ssn.merge(
            Player(
                id=user_id,
                joined_at_ts=date_ts,
                joined_at_txt=date_str,
                username=username,
                last_open=date_ts - 86500,
            )
        )
        await ssn.commit()
        ...
        if res.username != username:
            await ssn.execute(
                update(Player).filter(Player.id == res.id).values(username=username)
            )
            await ssn.commit()
...
async def get_top_rating(ssn: AsyncSession, user_id):
    ...
    user_top_q = await ssn.execute(
        select(Player.id)
        .filter(Player.season_rating >= user.season_rating)
...
async def get_all_time_rating(ssn: AsyncSession, user_id):
    top_q = await ssn.execute(select(Player).order_by(Player.rating.desc()).limit(10))
    top = top_q.scalars().all()
    ...
    user_top_q = await ssn.execute(
        select(Player.id)
        .filter(Player.rating >= user.rating)
        .filter(Player.id != user_id)
    )
    user_top = user_top_q.scalars().all()
    place = len(user_top) + 1
...
async def get_all_time_penalty(ssn: AsyncSession, user_id):
    ...
    user_top_q = await ssn.execute(
        select(Player.id)
        .filter(Player.penalty_rating >= user.penalty_rating)
        .filter(Player.id != user_id)
    )
    user_top = user_top_q.scalars().all()
    place = len(user_top) + 1
...
async def update_user_info(ssn: AsyncSession, user_id):
    ...
    for card in cards:
        ...
        if card.points != card.card.points:
            await ssn.execute(
                ...
                .values(points=card.card.points, card_rarity=card.card.rarity)
            )
    ...
    await ssn.execute(
        update(Player)
        .filter(Player.id == user_id)
        .values(rating=rating, card_quants=len(cards))
    )
    await ssn.commit()
...
async def get_or_add_userpacks(ssn: AsyncSession, user_id):
    upacks_q = await ssn.execute(select(UserPacks).filter(UserPacks.id == user_id))
...
async def promocode_fint_pass(ssn: AsyncSession, user_id):
    ...
    cards_q = await ssn.execute(
        select(CardItem)
        .filter(CardItem.rarity == "Limited")
    ...
    await ssn.execute(
        update(Games)
        .filter(Games.user_id == user_id)
        .filter(Games.kind == "darts")
        .values(last_free=Games.last_free - 172800)
    )
    ...
    new_pick = await ssn.merge(
        PlayerPick(
            user_id=user_id,
            card_one=pick_cards[0].id,
            card_two=pick_cards[1].id,
            card_three=pick_cards[2].id,
        )
    )
    await ssn.commit()
    pick_id = new_pick.id
