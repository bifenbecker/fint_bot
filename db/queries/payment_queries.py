...
from db.models import CardItem, Games, PayItem, Player, PlayerPick, UserCard, UserPacks
...
async def cancel_payment(ssn: AsyncSession, pay_id, user_id):
    await ssn.execute(
        update(PayItem).filter(PayItem.id == pay_id).values(status="canceled")
    )
    await ssn.commit()
...
async def add_new_payment(ssn: AsyncSession, user_id, label, url, kind, amount):
    pay = await ssn.merge(
        PayItem(label=label, url=url, user_id=user_id, amount=amount, kind=kind)
    )
    await ssn.commit()
    return pay.id
...
async def add_ls_after_pay(ssn: AsyncSession, user_id, pay_id, quant):
    await ssn.execute(
        update(PayItem).filter(PayItem.id == pay_id).values(status="paid")
    )
...
async def add_player_pick_pack(ssn: AsyncSession, user_id, pay_id):
    ...
    await ssn.execute(
        ...
        .values(player_pick=UserPacks.player_pick + 1)
    )
    await ssn.execute(
        update(Player)
        .filter(Player.id == user_id)
        .values(transactions=Player.transactions + 1)
    )
    await ssn.commit()
    return "done"
...
async def add_cards_pack(ssn: AsyncSession, user_id, quant, pay_id):
    ...
    if quant == 30:
        ...
        await ssn.execute(
            update(UserPacks)
            .filter(UserPacks.id == user_id)
            .values(thirty_pack=UserPacks.thirty_pack + 1)
        )
    elif quant == 20:
        await ssn.execute(
            update(UserPacks)
...
async def add_darts_after_pay(ssn: AsyncSession, user_id, pay_id, quant):
    ...
    await ssn.execute(
        ...
        .values(attempts=Games.attempts + quant)
    )
    await ssn.execute(
        update(Player)
        .filter(Player.id == user_id)
...
async def add_casino_after_pay(ssn: AsyncSession, user_id, pay_id, quant):
    ...
    await ssn.execute(
        update(Games)
        .filter(Games.id == game.id)
        .values(curr_casino=Games.curr_casino + quant * 3)
    )
    await ssn.execute(
        update(Player)
        .filter(Player.id == user_id)
