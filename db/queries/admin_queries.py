...
async def update_card_image(ssn: AsyncSession, card_id, image):
    await ssn.execute(
        update(CardItem).filter(CardItem.id == card_id).values(image=image)
    )
    await ssn.commit()
...
async def update_card_text(
    ...
    await ssn.execute(
        ...
        .values(points=points, card_rarity=rarity)
    )
    await ssn.commit()
...
async def add_new_promo(ssn: AsyncSession, card_id, text, kind, uses, users):
    await ssn.merge(
        PromoCode(promo=text, card_id=card_id, kind=kind, quant=uses, users=users)
    )
    await ssn.commit()
...
async def update_card_status(ssn: AsyncSession, card_id, status, rarity):
    await ssn.execute(
        update(CardItem).filter(CardItem.id == card_id).values(status=status)
    )
    await ssn.commit()
...
async def ban_user(ssn: AsyncSession, username, status):
    ...
    await ssn.execute(
        update(Player).filter(Player.id == user_id).values(ban_status=status)
    )
    await ssn.commit()
