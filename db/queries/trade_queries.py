...
async def update_trade_status(ssn: AsyncSession, user_id, card_id, trade_id):
    ...
    if trade.status == "target_wait":
        await ssn.execute(
            update(Trade)
            .filter(Trade.id == trade.id)
            .values(target_card_id=card_id, status="owner_wait")
        )
        await ssn.commit()
        return trade, cards[0].card
...
async def decline_last_trade(ssn: AsyncSession, user_id):
    ...
    await ssn.execute(
        update(Trade).filter(Trade.id == trade_res[0].id).values(status="canceled")
    )
    await ssn.commit()
...
async def close_trade(ssn: AsyncSession, trade_id):
    ...
    owner_cards_q = await ssn.execute(
        select(UserCard)
        .filter(UserCard.card_id == trade.owner_card_id)
    ...
    target_cards_q = await ssn.execute(
        select(UserCard)
        .filter(UserCard.card_id == trade.target_card_id)
    ...
    if (len(owner_cards) == 0) or (len(target_cards) == 0):
        await ssn.execute(
            update(Trade).filter(Trade.id == trade_id).values(status="error")
        )
        await ssn.commit()
        return "error"
    ...
    target_card_q = await ssn.execute(
        select(UserCard)
        .filter(UserCard.user_id == trade.owner)
        .filter(UserCard.card_id == target_card.card_id)
    )
    ...
    await ssn.execute(
        update(UserCard)
        .filter(UserCard.id == target_card.id)
        .values(user_id=trade.owner, duplicate=owner_duplicate)
    )

    # Добавляем карту первого игрока второму игроку
    owner_card_q = await ssn.execute(
        select(UserCard)
        .filter(UserCard.user_id == trade.target)
        .filter(UserCard.card_id == owner_card.card_id)
    )
    ...
    await ssn.execute(
        update(UserCard)
        .filter(UserCard.id == owner_card.id)
...
async def get_trade_access_user_by_trade_count(ssn: AsyncSession, user_id):
    ...
    access: str = (
        "yes" if access_q.trade_count < 5 or access_q.trade_count is None else "not"
    )
    return access
