...
from db.models import (CardItem, CardPack, CardXPack, Games, Player,
                       PlayerPick, UserCard)
    ...
    if user.last_lucky < date_ts:
        ...
        if value < 3:
            ...
            await ssn.execute(update(Player).filter(
                Player.id == user_id).values(last_lucky=date_ts + delay))
            ...
            cards_q = await ssn.execute(select(CardItem).filter(
                CardItem.rarity == rarity).filter(CardItem.status == "on"))
...
async def get_game(ssn: AsyncSession, user_id, kind):
    game_q = await ssn.execute(select(Games).filter(
    ...
    if kind == "free":
        if time == 1:
            ...
            await ssn.execute(update(Games).filter(
                Games.id == game.id).values(last_free=new_ts, free_quant=2))
        else:
            await ssn.execute(update(Games).filter(
                Games.id == game.id).values(free_quant=Games.free_quant - 1))
    else:
        await ssn.execute(update(Games).filter(
            Games.id == game.id).values(attempts=Games.attempts - 1))
    await ssn.commit()
    return res
