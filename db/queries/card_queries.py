...
from db.models import CardItem, Player, PromoCode, PromoUser, UserCard, UserPacks
...
async def use_promo(ssn: AsyncSession, user_id, promocode):
    ...
    promo_user_q = await ssn.execute(
        select(PromoUser.id)
        .filter(PromoUser.promo_id == promo.id)
        .filter(PromoUser.user_id == user_id)
    )
    ...
    if promo.quant <= 1:
        ...
        await ssn.execute(
            ...
            .values(quant=PromoCode.quant - 1)
        )
