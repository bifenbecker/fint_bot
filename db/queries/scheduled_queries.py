...
async def get_free_card_notity_users(db, now_ts):
    ...
    async with db() as ssn:
        card_users_q = await ssn.execute(
            select(Player)
            .filter(Player.last_open > now_ts)
            .filter(Player.last_open < next_ts)
        )
        card_users = card_users_q.scalars().all()

        ls_users_q = await ssn.execute(
            select(Player)
            .filter(Player.last_lucky > now_ts)
            .filter(Player.last_lucky < next_ts)
        )
        ls_users = ls_users_q.scalars().all()

        darts_users_q = await ssn.execute(
            select(Games)
            .filter(Games.kind == "darts")
            .filter(Games.last_free > now_ts)
            .filter(Games.last_free < next_ts)
        )
...
async def get_banned_users(db):
    ssn: AsyncSession
    async with db() as ssn:
        banned_q = await ssn.execute(
            select(Player.id).filter(Player.ban_status == "banned")
        )
...
async def get_seven_notify_users(db):
    ...
    async with db() as ssn:
        users_q = await ssn.execute(
            select(Player.id)
            .filter(Player.last_open <= old_date_ts)
            .filter(Player.seven_days == 1)
        )
        users = users_q.scalars().all()

        await ssn.execute(
            update(Player)
            .filter(Player.id.in_(users))
            .values(seven_days=0, lucky_quants=Player.lucky_quants + 5)
        )
        await ssn.commit()
...
async def get_fourteen_notify_users(db):
    ...
    async with db() as ssn:
        ...
        await ssn.execute(
            update(Player)
            .filter(Player.id.in_(users))
            .values(fourteen_days=0, lucky_quants=Player.lucky_quants + 10)
        )
        await ssn.commit()
    ...
    async with db() as ssn:
        ...
        for top_r in top_rs:
            await ssn.merge(
                Season(
                    user_id=top_r.id,
                    username=top_r.username,
                    kind="rating",
                    rating=top_r.season_rating,
                    date_str=date_str,
                )
            )
        await ssn.commit()
        ...
        for top_p in top_ps:
            await ssn.merge(
                Season(
                    user_id=top_p.id,
                    username=top_p.username,
                    kind="penalty",
                    rating=top_p.season_penalty,
                    date_str=date_str,
                )
            )
        await ssn.commit()
...
async def remove_premium_from_users(db, date_ts):
    ...
    async with db() as ssn:
        users_q = await ssn.execute(
            select(Player.id)
            .filter(Player.pass_ts <= date_ts)
            .filter(Player.game_pass == "yes")
        )
        users = users_q.scalars().all()
        ...
        if len(users) > 0:
            await ssn.execute(
                update(Player)
                .filter(Player.id.in_(users))
                .values(game_pass="no", pass_ts=0, pass_until="nopass")
            )
            await ssn.commit()
