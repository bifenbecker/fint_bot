...
from db.models import (CardItem, CardPack, CardXPack, PackBattle, Player,
                       UserCard, UserPacks)
...
async def get_active_pack_battle_lobbies(ssn: AsyncSession):
    battles_q = await ssn.execute(select(PackBattle).filter(
        PackBattle.target == 0).filter(
            PackBattle.status == "active").order_by(PackBattle.quant))
    battles = battles_q.scalars().all()
    return battles


async def check_for_pack_battle_available(ssn: AsyncSession, user_id):
    battle_q = await ssn.execute(select(PackBattle).filter(
        or_(PackBattle.target == user_id, PackBattle.owner == user_id)).filter(
            PackBattle.status == "active"))
...
async def create_leg_pack_battle(ssn: AsyncSession, user_id, msg_id, username):
    ...
    new_battle = await ssn.merge(PackBattle(
        owner_msg_id=msg_id, owner_points=0,
        owner=user_id, owner_username=username, quant=0))
    await ssn.execute(update(UserPacks).filter(
        UserPacks.id == user_id).values(leg_pack=UserPacks.leg_pack - 1))
    await ssn.commit()

    logging.info(
        f"User {user_id} ({username}) created leg pack battle lobby {new_battle.id}")
    battle_q = await ssn.execute(select(PackBattle).filter(
        PackBattle.id == new_battle.id))
    battle = battle_q.fetchone()[0]
    return battle


async def create_default_pack_battle(ssn: AsyncSession, user_id, msg_id, username, quant):
    battle_q = await ssn.execute(select(PackBattle).filter(
        or_(PackBattle.target == user_id, PackBattle.owner == user_id)).filter(
            PackBattle.status == "active"))
    battle_res = battle_q.fetchone()
    ...
    new_battle = await ssn.merge(PackBattle(
        owner_msg_id=msg_id, owner_points=0,
        owner=user_id, owner_username=username, quant=quant))
...
async def owner_card_battle_cancel(ssn: AsyncSession, battle_id):
    ...
    await ssn.execute(update(PackBattle).filter(
        PackBattle.id == battle_id).values(status="canceled"))

    if battle.quant == 20:
        await ssn.execute(update(UserPacks).filter(
            UserPacks.id.in_([battle.owner, battle.target])).values(
...
async def target_card_battle_cancel(ssn: AsyncSession, battle_id, user_id):
    ...
    await ssn.execute(update(PackBattle).filter(
        PackBattle.id == battle_id).values(
            target=0, target_username="nouser",
            target_msg_id=0, target_ts=0))

    if battle.quant == 20:
        await ssn.execute(update(UserPacks).filter(
            UserPacks.id == battle.target).values(
                twenty_pack=UserPacks.twenty_pack + 1))
    elif battle.quant == 10:
        await ssn.execute(update(UserPacks).filter(
            UserPacks.id == battle.target).values(
...
async def update_owner_battle_msg_id(ssn: AsyncSession, battle_id, msg_id):
    await ssn.execute(update(PackBattle).filter(
        PackBattle.id == battle_id).values(owner_msg_id=msg_id))
    await ssn.commit()


async def update_target_battle_msg_id(ssn: AsyncSession, battle_id, msg_id):
    await ssn.execute(update(PackBattle).filter(
        PackBattle.id == battle_id).values(target_msg_id=msg_id))
    await ssn.commit()
        ...
        if battle.status == "active":
            if kind == "owner":
                if date_ts == battle.owner_ts:
                    await ssn.execute(update(PackBattle).filter(
                        PackBattle.id == battle_id).values(status="timedout"))
                ...
                if (user_id == battle.target) and (date_ts == battle.target_ts):
                    ...
                    if battle.quant == 20:
                        await ssn.execute(update(UserPacks).filter(
                            UserPacks.id == old_user_id).values(
                                twenty_pack=UserPacks.twenty_pack + 1))
                    elif battle.quant == 10:
                        await ssn.execute(update(UserPacks).filter(
                            UserPacks.id == old_user_id).values(
                    ...
                    return battle, "target_timeout", old_user_id, old_msg_id, old_username
                else:
                    return battle, "continued"
...
async def battle_user_ready(ssn: AsyncSession, user_id, battle_id):
    ...
    if battle.owner == user_id:
        if battle.target_ready == 0:
            await ssn.execute(update(PackBattle).filter(
                PackBattle.id == battle_id).values(owner_ready=1))
            await ssn.commit()
            ...
            if battle.quant == 0:
                ...
                owner_result = await calc_default_pack(
                    ssn, battle.owner, battle.quant)
            ...
            if owner_result[0] == target_result[0]:
                ...
                for card_id in owner_result[1]:
                    ...
                    usercard_q = await ssn.execute(select(UserCard).filter(
                        UserCard.user_id == battle.owner).filter(
                    ...
                    user_card = await ssn.merge(UserCard(
                        user_id=battle.owner, card_id=card.id, points=card.points,
                ...
                for card_id in target_result[1]:
                    ...
                    card: CardItem = card_q.fetchone()[0]
                    usercard_q = await ssn.execute(select(UserCard).filter(
                        UserCard.user_id == battle.target).filter(
                            UserCard.card_id == card.id))
                    user_card_res = usercard_q.fetchone()
                    if user_card_res is None:
                        duplicate = 0
                    ...
                    user_card = await ssn.merge(UserCard(
                        user_id=battle.target, card_id=card.id, points=card.points,
                        card_rarity=card.rarity, duplicate=duplicate))
                    await ssn.commit()
                ...
                await ssn.execute(update(Player).filter(
                    Player.id == battle.target).values(
                ...
                for card_id in owner_result[1]:
                    ...
                    card: CardItem = card_q.fetchone()[0]
                    usercard_q = await ssn.execute(select(UserCard).filter(
                        UserCard.user_id == winner).filter(
                            UserCard.card_id == card.id))
                    user_card_res = usercard_q.fetchone()
                    if user_card_res is None:
                        duplicate = 0
                    ...
                    user_card = await ssn.merge(UserCard(
                        user_id=winner, card_id=card.id, points=card.points,
                        card_rarity=card.rarity, duplicate=duplicate))
                    await ssn.commit()
                    ownr_u_cards.append(user_card.id)
                    points += card.points
            ...
            await ssn.execute(update(PackBattle).filter(
                PackBattle.id == battle_id).values(
                    owner_points=owner_result[0], owner_ts=0,
                    target_points=target_result[0], target_ts=0,
...
async def calc_leg_pack(ssn: AsyncSession, user_id):
    leg_cards_q = await ssn.execute(select(CardItem).filter(
        CardItem.rarity == "ЛЕГЕНДАРНАЯ").filter(
            CardItem.status == "on"))
