...
async def save_player_pick(ssn: AsyncSession, user_id, pick_id, card_id):
    pick_q = await ssn.execute(select(PlayerPick).filter(PlayerPick.id == pick_id))
    pick: PlayerPick = pick_q.fetchone()[0]

    if pick.card_pick != 0:
        return "not_active"

    card_q = await ssn.execute(select(CardItem).filter(CardItem.id == card_id))
    ...
    await ssn.execute(
        update(PlayerPick).filter(PlayerPick.id == pick_id).values(card_pick=card_id)
    )
    await ssn.execute(
        update(Player)
        .filter(Player.id == user_id)
    ...
    logging.info(f"User {user_id} pick card {card_id} from player pick {pick_id}")
