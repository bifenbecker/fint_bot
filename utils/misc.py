...
from keyboards.games_kbs import (
    card_penalty_answer_kb,
    card_penalty_offer_kb,
    penalty_action_kb,
    penalty_offer_kb,
)
...
async def card_rarity_randomize(kind):
    if kind == "card":
        ...
        rarities = ["Unique", "Flashback", "Legendary", "Moments", "Heroes", "Icons"]
...
async def send_card_penalty_answer(bot: Bot, user_id, username, pen_id, image):
    ...
    try:
        msg = await bot.send_photo(
            user_id, image, caption=txt, reply_markup=card_penalty_answer_kb(pen_id)
        )
        msg_id = msg.message_id
    except Exception as error:
        logging.error(f"Send error | chat {user_id}\n{error}")
...
async def send_penalty_action(bot: Bot, user_id, pen_id, kind):
    if kind == "kicker":
        txt = "Выбери угол, в который хочешь ударить"
    else:
        txt = "Выбери угол, в который хочешь прыгнуть"
