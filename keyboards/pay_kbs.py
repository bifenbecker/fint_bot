...
def player_pick_kb(page, last, pick_id, card_id):
    btns = [
        [
            InlineKeyboardButton(text="Выбрать игрока",
                                 callback_data=f"plrpick_{pick_id}_{card_id}")
        ]
    ]
...
buy_pass_kb = InlineKeyboardMarkup(
    inline_keyboard=[
        [
            InlineKeyboardButton(
                text="Купить FINT Pass", callback_data="buyfintpass")
        ],
        [
            InlineKeyboardButton(
                text="Активировать промокод", callback_data="promo")
        ],
        [
            InlineKeyboardButton(
