...
from db.queries.admin_queries import (
    update_card_image,
    update_card_status,
    update_card_text,
)
...
async def paginate_adm_rarity_cards_cmd(c: CQ, state: FSM, callback_data: PageCB):
    ...
    try:
        await c.message.edit_media(
            media=media,
            reply_markup=adm_view_cards_kb(page, last, card.id, kind, card.status),
        )
...
@router.callback_query(F.data.startswith("imgedit_"), IsAdmin(), flags=flags)
...
async def save_new_card_image_cmd(m: Mes, state: FSM, ssn):
    ...
    await m.answer("✅ Новое фото для карточки установлено!", reply_markup=admin_kb)


@router.callback_query(F.data.startswith("txtedit_"), IsAdmin(), flags=flags)
...
async def save_new_card_text_cmd(m: Mes, state: FSM, ssn):
    ...
    if len(data) != 6:
        ...
        if rarity not in rarities:
            ...
            await m.answer(
                "✅ Новые характеристики для карточки установлены!",
                reply_markup=admin_kb,
            )


@router.callback_query(F.data.startswith("chngstatus_"), IsAdmin(), flags=flags)
