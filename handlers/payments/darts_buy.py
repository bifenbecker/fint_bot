...
from db.queries.payment_queries import (add_darts_after_pay, add_new_payment,
                                        get_payment_info)
...
async def buy_darts_cmd(c: CQ, wallet, ssn):
    ...
    pay_res = await create_new_bill(
        price, c.from_user.id, f"{quant} Darts", wallet)
    ...
    await c.message.edit_text(txt, reply_markup=pay_kb(pay_id, pay_res[1], f"dts{quant}"))


@router.callback_query(PayCB.filter((F.act == "paid") & (F.kind.startswith("dts"))), flags=flags)
