    ...
    if len(cards) == 0:
        ...
        await c.message.answer_photo(
            ...
            reply_markup=target_card_trade_kb(
                page, last, sorting, cards[0].card_id, trade_id
            ),
        )
...
async def view_owner_trade_rarity_cards_cmd(c: CQ, ssn, state: FSM):
    ...
    if len(cards) == 0:
        ...
        await state.set_state(UserStates.target_trade)
        await state.update_data(cards=cards, sorting="nosort", trade_id=trade_id)
...
async def cancel_all_trades_cmd(c: CQ, ssn):
    ...
    await c.message.edit_text(
        "Вы успешно завершили все активные обмены", reply_markup=after_trade_kb
    )
