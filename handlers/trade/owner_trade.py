...
from db.queries.trade_queries import (
    check_target_trade,
    create_new_trade,
    decline_trade,
    get_trade_access_user,
    get_trade_access_user_by_trade_count,
    get_trade_card_rarities,
)
...
async def trade_cmd(c: CQ, state: FSM, ssn):
    ...
    if access_status == "not":
        ...
        return
    trade_count_access: str = await get_trade_access_user_by_trade_count(
        ssn, c.from_user.id
    )
    if trade_count_access == "not":
        ...
        await c.message.answer("🎭 Выбери формат обмена", reply_markup=trade_kb)
...
@router.callback_query(F.data.startswith("sorttrd_"), flags={"throttling_key": "pages"})
...
@router.callback_query(F.data.startswith("trdrar_"), flags={"throttling_key": "pages"})
async def view_owner_trade_rarity_cards_cmd(c: CQ, ssn, state: FSM):
    ...
    if len(cards) == 0:
        ...
        await c.message.answer_photo(
            cards[0].card.image,
            txt,
            reply_markup=card_trade_kb(page, last, "nosort", cards[0].card_id),
        )
...
async def paginate_owner_trade_cards_cmd(c: CQ, state: FSM, callback_data: PageCB):
    ...
    try:
        await c.message.edit_media(
            media=media, reply_markup=card_trade_kb(page, last, sorting, card.card_id)
        )
...
async def save_target_trade_username_cmd(m: Mes, state: FSM, ssn, bot: Bot):
    ...
    res = await create_new_trade(ssn, m.from_user.id, username, card_id, target)
    if res == "no_card":
        await m.answer(
